...
class FaceRecognitionSystem:
    ...
    def _calculate_similarity(self, embedding1: np.ndarray, embedding2: np.ndarray) -> float:
        ...
        try:
            # Ensure embeddings are the same dtype and length
...
def register_employee_endpoint():
    ...
    # Call the register_employee method with numpy image
...
def recognize_employee_endpoint():
    ...
    # Convert uploaded file → PIL → NumPy array
    try:
...
def attendance():
    ...
    # Get the best match by confidence
    ...
    attendance_result = face_system.log_attendance(
        ...
        action,
        float(best_match["confidence"])  # ensure JSON serializable
    )
